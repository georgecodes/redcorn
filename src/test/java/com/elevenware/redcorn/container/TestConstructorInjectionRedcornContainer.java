...
import com.elevenware.redcorn.hierarchy.*;
...
public class TestConstructorInjectionRedcornContainer {
    ...
    public void canStillAddBeansAfterContainerStarted() {
        ...
        container.register("before", NamedBean.class).addConstructorArg("this is injected prior to start");
        container.start();
        container.register("after", NamedBean.class).addConstructorArg("this is injected after start");
        container.refresh();

        NamedBean bean = container.find("after");
        assertNotNull(bean);
